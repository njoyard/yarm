...
function assertDocArrayCollection(res, body, fieldvalues) {
  let docArray = testData[3].docArray;
...
describe("Mongoose resources", function() {
  ...
  describe("Model resources", function() {
    describe("Model collections", function() {
      ...
      function queryTest(query, expected, done) {
        ...
        request.get(`/test?query=${encodeURIComponent(query)}`, function(
          res,
          body
        ) {
      ...
      it(
        ...
        queryTest.bind(null, "field1:/o/ OR field2:/a/", [
          "foo",
          "bar",
          "urldecode"
        ])
      ...
      it(
        ...
        queryTest.bind(null, "field1:/o/ OR field1:/a/ AND field2:/a/", [
          "foo",
          "bar",
          "urldecode"
        ])
      ...
      it("should allow setting the default sort with an option", function(done) {
        mongooseResource("test", TestModel).set("sort", { field1: "asc" });
    ...
    describe("Documents", function() {
      ...
      it(
        ...
        composeTests(
          testData.map(function(item) {
            return function(done) {
              mongooseResource("test", TestModel).set("toObject", {
                virtuals: true
              });
              ...
              request.get(`/test/${item._id}`, function(res, body) {
                ...
                assert.strictEqual(
                  doc.description,
                  `Document ${item.field1} with ${
                    item.docArray.length
                  } sub-documents`
                );
      ...
      it(
        ...
        composeTests(
          testData.map(function(item) {
            return function(done) {
              mongooseResource("test", TestModel).set("key", "field1");
      ...
      it("should PUT documents", function(done) {
        ...
        request.put(`/test/${item._id}`, { field2: "bar" }, function(
          res,
          body
        ) {
          assertEmpty(body);
    ...
    describe("Document fields", function() {
      ...
      it("should URLdecode field names", function(done) {
        ...
        request.get(
          `/test/${testData[4]._id}/url%20encoded%2Fproperty`,
          function(res, body) {
            assert.strictEqual(res.statusCode, 200);
      ...
      it("should 404 on nonexistent document fields", function(done) {
        ...
        request.get(`/test/${testData[0]._id}/nonexistent`, function(
          res,
          body
        ) {
          assert.strictEqual(res.statusCode, 404);
      ...
      it("should DELETE array field items in documents", function(done) {
        ...
        let item = testData.filter(function(item) {
    ...
    describe("DocumentArray collections", function() {
      ...
      function queryTest(query, expected, done) {
        ...
        request.get(
          `/test/${item._id}/docArray?query=${encodeURIComponent(query)}`,
          function(res, body) {
      ...
      it(
        "should allow queries with both AND and OR operators",
        queryTest.bind(null, "field:/o/ OR field:/z/ AND field:/^b/", [
          "foo",
          "baz"
        ])
      );
      ...
      it("should POST new documents to DocumentArray collections", function(done) {
        ...
        request.post(`/test/${item._id}/docArray`, { field: "bang" }, function(
          res,
          body
        ) {
          assertCreated(res, body);
          ...
          TestModel.findById(item._id, function(err, doc) {
            ...
            let subs = doc.docArray.filter(function(sub) {
      ...
      it("should POST new documents to DocumentArrays at specified index", function(done) {
        ...
        request.post(
          `/test/${item._id}/docArray?index=1`,
          { field: "bang" },
          function(res, body) {
            assertCreated(res, body);
            assert.strictEqual(res.statusCode, 201);
    ...
    describe("DocumentArray documents", function() {
      it(
        "should GET documents in DocumentArrays",
        composeTests(
          testData[3].docArray.map(function(item) {
            return function(done) {
              mongooseResource("test", TestModel);

              request.get(
                `/test/${testData[3]._id}/docArray/${item._id}`,
                function(res, body) {
                  let doc = assertJSON(body);
        ...
        composeTests(
          testData[3].docArray.map(function(item) {
            return function(done) {
              ...
              request.get(
                `/test/${testData[3]._id}/docArray/${item.field}`,
                function(res, body) {
                  let doc = assertJSON(body);
      ...
      it("should DELETE documents in DocumentArray collections", function(done) {
        ...
        request.del(`/test/${item._id}/docArray/${sub._id}`, function(
          res,
          body
        ) {
          assertEmpty(body);
      ...
      it("should PUT documents in DocumentArray collections", function(done) {
        ...
        request.put(
          `/test/${item._id}/docArray/${sub._id}`,
          { field: "bang" },
          function(res, body) {
            assertEmpty(body);
            assert.strictEqual(res.statusCode, 204);
    ...
    describe("DocumentArray document fields", function() {
      it(
        "should GET fields in documents in DocumentArrays",
        composeTests(
          testData[3].docArray.map(function(item, index) {
            return function(done) {
              mongooseResource("test", TestModel);

              request.get(
                `/test/${testData[3]._id}/docArray/${item._id}/field`,
                function(res, body) {
                  assert.strictEqual(res.statusCode, 200);
      ...
      it("should DELETE fields in documents in DocumentArrays", function(done) {
        ...
        request.del(`/test/${item._id}/docArray/${sub._id}/field`, function(
          res,
          body
        ) {
          assertEmpty(body);
      ...
      it("should PUT fields in documents in DocumentArrays", function(done) {
        ...
        request.put(
          `/test/${item._id}/docArray/${sub._id}/field`,
          { _value: "bang" },
          function(res, body) {
            assertEmpty(body);
            assert.strictEqual(res.statusCode, 204);
  ...
  describe("Aggregate resources", function() {
    ...
    it(
      "should allow queries with both AND and OR operators",
      aggregateTest.bind(null, "parent:/r/ OR _id:/a/ AND _id:/b/", [
        "foo",
        "baz",
        "bar"
      ])
