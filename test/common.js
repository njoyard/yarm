...
function allMethods(it, init, path, callback) {
  let call = ["get", "put", "post", "del", "patch"];
...
function callbackTests(method, it) {
  ...
  if (doResultTests) {
    it(`should respond with ${noContent.code} ${
      noContent.msg
    } when ${methodName} handler sends nothing`, function(done) {
      let r = resource("test");
    ...
    it(`should send response with mimetype from ${methodName} handler`, function(done) {
      ...
      doRequest("/test", function(res, body) {
        assert.strictEqual(
          ...
          "text/x-test-content; charset=utf-8"
        );
        assert.strictEqual(body, "Test content");
