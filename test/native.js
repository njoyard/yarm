...
function allTypes(description, doRequest, checkResponse, options) {
  ...
  Object.keys(types).forEach(function(type) {
    it(description.replace("%type", type), function(done) {
      let expectedValue = types[type].value,
      ...
      doRequest(expectedValue, jsonBody, function(res, body) {
        let actualValue = checkResponse(expectedValue, res, body);
        ...
        if (type === "array") {
          ...
          if (!options.objectCollections) {
            assert.strictEqual(typeof actualValue, "object");
            assert.strictEqual(
              Object.keys(actualValue).join(","),
              Object.keys(expectedValue).join(",")
            );
            assert.strictEqual(
              objectValues(actualValue).join(","),
              objectValues(expectedValue).join(",")
            );
          } else {
            assert.strictEqual(typeof actualValue, "object");
            ...
            assert.strictEqual(
              actualValue._count,
              Object.keys(expectedValue).length
            );
...
describe("Native resources", function() {
  describe("Root resources", function() {
    ...
    it("Should GET arrays as collections", function(done) {
      resource("test", ["foo", "bar", "baz"]).set("arrayCollections", true);
    ...
    it("Should GET array resources as is when rawArrays is true", function(done) {
      resource("test", ["foo", "bar", "baz"]).set("rawArrays", true);
    ...
    it("Should POST new key/value pairs to object resources", function(done) {
      ...
      request.post("/test", { _key: "added", _value: "baz" }, function(
        res,
        body
      ) {
  ...
  describe("Properties", function() {
    describe("GET", function() {
      allTypes(
        "Should GET %type property values",
        function(expected, json, callback) {
    ...
    describe("PUT", function() {
      let obj = { property: "foo" };

      allTypes(
        "Should PUT %type property values",
        function(expected, json, callback) {
          resource("test", obj);
    ...
    describe("PATCH", function() {
      it("Should PATCH object property values", function(done) {
        ...
        request.patch("/test/obj", { number: 42, string: "foo" }, function(
          res,
          body
        ) {
          assert.strictEqual(res.statusCode, 204);
      ...
      it("Should PATCH object array items", function(done) {
        ...
        request.patch("/test/0", { number: 42, string: "foo" }, function(
          res,
          body
        ) {
          assert.strictEqual(res.statusCode, 204);
    ...
    describe("POST", function() {
      it("Should POST new key/value pairs to object sub-resources", function(done) {
        ...
        request.post("/test/obj", { _key: "added", _value: "baz" }, function(
          res,
          body
        ) {
          assert.strictEqual(res.statusCode, 201);
      ...
      it("Should refuse to POST to object sub-resources with non-string _key in input", function(done) {
        ...
        request.post("/test/obj", { _key: 1, _value: "baz" }, function(
          res,
          body
        ) {
          assert.strictEqual(res.statusCode, 400);
