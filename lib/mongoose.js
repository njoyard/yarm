...
# Setting fields in subdocs

doc.subdoc.field = "value"; // OK
doc.set("subdoc.field", "value"); // OK
doc.subdoc.set("field", "value"); // No error but does not change value

# Setting fields in doc arrays

doc{.arr,.get("arr")}[i].field = "value"; // OK
doc.arr.id(ObjectId).field = "value; // OK
doc.arr.id(ObjectId).set("field", "value") // OK
 ...
 *  "toObject"  mongoose toObject options
 */
...
MongooseDocumentElement.prototype._set = function(value) {
	...
	if (chain.length === 0) {
		// Empty chain, this is a root document
		current.set(value);
		return;
	}

	// Follow chain until last link
	var last = chain.pop();
	chain.forEach(function(link) {
		if (link instanceof ObjectId) {
			current = current.id(link);
	...
	// Set value using last link
	if (last instanceof ObjectId) {
		current.id(last).set(value);
...
MongooseDocumentElement.prototype.sub = function(id, cb) {
	...
	if (sub) {
		...
		if (typeof sub.link === "string" && typeof chain[chain.length - 1] === "string") {
			chain[chain.length - 1] += "." + sub.link;
		} else {
			chain.push(sub.link);
		}
...
MongooseDocumentArray.prototype._setSubPath = function(path, value) {
	var elem = this._elem.id(path),
		index = this._elem.indexOf(elem);

	this._elem.splice(index, 1, value);
};

MongooseDocumentArray.prototype._unsetSubPath = function(path) {
	var elem = this._elem.id(path),
		index = this._elem.indexOf(elem);

	this._elem.splice(index, 1);
};
