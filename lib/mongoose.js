...
/* Add Request object on a document according to the request option */
function addRequest(req, options, doc) {
	if (options.request !== false) {
		doc[options.request || "_request"] = req;
	}
}
	...
	return Object.keys(crit).every(function(path) {
		...
		if (typeof match === "string") {
			...
			if (match instanceof RegExp) {
				...
				throw new Error("Unsupported query criterion " + util.inspect(match));
			}
		}
...
MongooseDocument.prototype.get = function(req, cb) {
	addRequest(req, this._options, this._elem);
 ...
 *   "request"    by default, yarm will set a _request property on all documents
 *                it gets from mongoose to the current express Request object,
 *                which can then be used in virtual getters or document methods.
 *                This option enables changing the property name or disabling it
