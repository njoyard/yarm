...
function addHandler(compiled, method, handler) {
...
function addHook(compiled, hook, strict) {
...
function addOptions(compiled, options) {
	var item = Object.create(compiled);
...
"get list count post put del".split(" ").forEach(function(method) {
	Path.prototype[method] = function(handler) {
		addHandler(this.compiled, method, handler);
		return this;
	};
});
...
Path.prototype.hook = function(hook, strict) {
	addHook(this.compiled, hook, strict);
	return this;
};
...
Path.prototype.sub = function(pattern, hook) {
	...
	return root.sub(this.compiled.raw + "/" + pattern, hook);
};


Path.prototype.remove = function(pattern) {
	pattern = pattern.replace(regexpTrimSlashes, "");
	return root.remove(this.compiled.raw + "/" + pattern);
};
...
Path.prototype.set = function(key, value, strict) {
	...
	var hook = handlers.filter(function(h) {
		return h.options && h.raw === compiled.raw;
	})[0];

	if (!hook) {
		hook = addOptions(compiled, {});
	}
...
function getParamHook(names, match) {
	...
	return function paramHook(req, next) {
		...
		names.forEach(function(name) {
			req.params[name] = decodeURIComponent(values.shift());
		});
...
var root = {
	...
	remove: function(pattern) {
		...
		handlers = handlers.filter(filterFunc);
	},
