...
function mongoosePathHook(req, next) {
	...
	while(parts.length > 0) {
		...
		if (current instanceof DocumentArray) {
			...
			if (req.options.key && req.options.key !== "_id") {
				current = current.filter(matchItem)[0];
				link = { id: current._id };
			} else {
...
function mongooseResource(name, Model) {
	var collResource = root.sub(name)
	...
	var docResource = collResource.sub(":documentId")
